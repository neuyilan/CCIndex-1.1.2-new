...
/**
 * Simple index key Generator used most commonly.
...
public class SimpleIndexKeyGenerator implements IndexKeyGenerator {
	...
	public byte[] createIndexRowKey(byte[] rowKey, byte[] value) {
		...
		byte[] a = Bytes.toBytes(IndexConstants.LASTPART_ZERO.substring(0,
				IndexConstants.LASTPART_LENGTH - (""+value.length).length())
				+ value.length);
	...
	public byte[][] parseIndexRowKey(byte[] indexKey) {
		int length = Integer.valueOf(
				Bytes.toString(Bytes.tail(indexKey,
						IndexConstants.LASTPART_LENGTH))).intValue();
