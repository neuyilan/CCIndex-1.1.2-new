    ...
    // conn.getRegionServerWithRetries(
          ...
          if (!hri.containsRange(first, last)) {
            LOG.info("HFile at " + hfilePath + " no longer fits inside a single " +
              ...
              HColumnDescriptor familyDesc =
                  ((HConnection) this.connection).getHTableDescriptor(hri.getTable()).getFamily(item.family);
        ...
        Void succcess = 
					RpcRetryingCallerFactory.instantiate(
							conn.getConfiguration(),null).<Void> newCaller().callWithRetries(callable, Integer.MAX_VALUE);
