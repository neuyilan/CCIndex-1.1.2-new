...
import ict.ocrabase.main.java.client.bulkload.BulkLoad.NoNeedBuildIndexException;
...
/**
 * IndexAdmin maintains indexes of tables, such as create index table, 
 * add indexes and delete indexes.
 * 
 * @author wanhao
 */
public class IndexAdmin{
	private static final Log LOG = LogFactory.getLog(IndexAdmin.class);
	 ...
	 * @throws IOException 
	 */
	public IndexAdmin() throws IOException
	{
	 ...
	 * @throws IOException 
	 */
	public IndexAdmin(Configuration conf) throws IOException {
		this.conf=conf;
		this.admin=new HBaseAdmin(this.conf);
	}
	 ...
	 * @param admin
	 */
	public IndexAdmin(HBaseAdmin admin){
	 ...
	 * @throws IndexExistedException 
	 */
	public void addIndex(byte[] tableName, IndexSpecification indexSpec) throws IOException, IndexExistedException {
		if(indexSpec==null){
	 ...
	 * 
	 * @param tableName
	 * @param indexSpecs		if any IndexSpecification's TableName is not equal to tableName,
	 * 							it will be set to tableName.
	 * @throws IOException
	 * @throws IndexExistedException 
	 */
	public void addIndexes(byte[] tableName, IndexSpecification[] indexSpecs) throws IOException, IndexExistedException{
		...
		HTableDescriptor backupdesc=admin.getTableDescriptor(tableName);//backup, used in case of exceptions
		
		try{
			//create tables of indexes which want to be added
			for(IndexSpecification spec : indexSpecs){
				admin.createTable(indexDesc.createIndexTableDescriptor(spec.getIndexColumn()),spec.getSplitKeys());
    ...
    try {
      ...
      for (IndexSpecification index : indexSpecs) {
        tmp += Bytes.toString(index.getIndexColumn());
        if (count != indexSpecs.length - 1) {
          tmp += ",";
        }
        count++;
      }
      BulkLoad bk = new BulkLoad(conf, new TableInfo(Bytes.toString(tableName) + ",1," + tmp));
      bk.run();
      while (!bk.isComplete()) {
        Thread.sleep(1000);
      }
    } catch (NoNeedBuildIndexException e1) {
      LOG.debug(e1.getMessage());
    } catch (InterruptedException e2) {
      throw new IOException("Build index Interrupted when adding indexes!", e2);
    } catch (IOException e3) {
      throw new IOException("Build index failed when adding indexes!", e3);
    }
	 ...
	 */
	public void deleteIndex(byte[] tableName, byte[] indexedColumn) throws IOException, IndexNotExistedException {
	 ...
	 */
	public void deleteIndexes(byte[] tableName, byte[][] indexedColumns) throws IOException, IndexNotExistedException {
	 ...
	 * <p>This table is not needed to be disabled before calling this method, and
	 * after the return the table will be enabled.
	 * 
	 * @param tableName
	 * @throws IOException
	 */
   ...
   */
	 ...
	 */
  /*
   * public void fixupMissingIndexes(byte[] tableName) throws IOException{ IndexTableDescriptor
   * indexDesc=new IndexTableDescriptor(admin.getTableDescriptor(tableName));
   * if(!indexDesc.hasIndex()){ return; } try{ admin.disableTable(tableName); String tmp=null;
   * for(IndexSpecification indexSpec:indexDesc.getIndexSpecifications()){
   * if(admin.tableExists(indexSpec.getIndexTableName())){
   * admin.disableTable(indexSpec.getIndexTableName()); } else { try {
   * admin.createTable(indexDesc.createIndexTableDescriptor
   * (indexSpec.getIndexColumn()),indexSpec.getSplitKeys()); } catch (IndexNotExistedException e) {
   * //impossible here } admin.disableTable(indexSpec.getIndexTableName()); if(tmp==null){
   * tmp=Bytes.toString(indexSpec.getIndexColumn()); }else{
   * tmp+=","+Bytes.toString(indexSpec.getIndexColumn()); } } } if(tmp==null){return;}
   * if(isTest){return;} BulkLoad bk=new BulkLoad(Bytes.toString(tableName),tmp); bk.run();
   * while(!bk.isComplete()){ Thread.sleep(1000); } }catch(NoNeedBuildIndexException e1){
   * LOG.debug(e1.getMessage()); } catch (InterruptedException e2) { throw new
   * IOException("Rebuild index interrupted!",e2); } catch (IOException e3){ throw new
   * IOException("Rebuild index failed, cause message:"+e3.getMessage(), e3); } String errStr="";
   * for(IndexSpecification indexSpec:indexDesc.getIndexSpecifications()){
   * if(admin.tableExists(indexSpec.getIndexTableName())){
   * admin.enableTable(indexSpec.getIndexTableName()); } else {
   * errStr+="[column:"+Bytes.toString(indexSpec.getIndexColumn()) + ",type:" +
   * indexSpec.getIndexType().toString()+"]"; } } admin.enableTable(tableName); if
   * (!errStr.isEmpty()) { throw new IOException("Try to fix up missing index " + errStr +
   ...
   */
		...
		try{
			...
			for(IndexSpecification indexSpec:indexDesc.getIndexSpecifications()){
				if(admin.tableExists(indexSpec.getIndexTableName())){
					...
					errStr+="[column:"+Bytes.toString(indexSpec.getIndexColumn())
			...
			if (!errStr.isEmpty()) {
				throw new IOException("Try to give up missing index " + errStr
					+ " failed!",e1);
			}
		} finally{
			for(IndexSpecification indexSpec:indexDesc.getIndexSpecifications()){
	 ...
	 */
	public void setKeygenClass(byte[] tableName, String className) throws IOException, ClassNotFoundException {
		HTableDescriptor desc = admin.getTableDescriptor(tableName);
		IndexTableDescriptor indexDesc = new IndexTableDescriptor(desc);
	 ...
	 * @throws IOException 
	 */
	public void setColumnInfoMap(byte[] tableName, Map<byte[], DataType> columnTypes) throws IOException{
		HTableDescriptor desc = admin.getTableDescriptor(tableName);
    ...
    desc.setValue("DATA_FORMAT", sb.toString());// should be ImportConstants.DATA_FORMAT
    ...
    String tempInfo = admin.getTableDescriptor(tableName).getValue("DATA_FORMAT");// should be
                                                                                  // ImportConstants.DATA_FORMAT
	 ...
	 * If there is any index on this family, then an exception will be thrown out.
	 * 
	 ...
	 * @param tableName name of table
	 * @param columnName name of column to be deleted
	 * @param force when there is any index on this family, if force is true, then delete index at the same time, 
	  				otherwise an exception will be thrown out.
	 * @throws IOException if a remote or network exception occurs
	 */
	  public void deleteColumn(final String tableName, final String columnName, final boolean force) throws IOException {
	   ...
	   * @throws IOException if a remote or network exception occurs
	   */
	  public void deleteColumn(final byte [] tableName, final byte [] columnName, final boolean force) throws IOException {
		if(isTableEnabled(tableName)){
			throw new IOException("Table "+Bytes.toString(tableName)+" is enabled! Disable it first!");
		}
			...
			for(IndexSpecification indexSpec:indexDesc.getIndexSpecifications()){
				if(Bytes.compareTo(indexSpec.getFamily(),columnName)==0){
					throw new IOException("Index [column:"+Bytes.toString(indexSpec.getIndexColumn())
						+ ",type:" + indexSpec.getIndexType().toString()+"] is already existed on column family "
							+ Bytes.toString(columnName)+". Please delete index first!");
				}
			}
		}
		
		admin.deleteColumn(Bytes.toString(tableName), Bytes.toString(columnName));
		...
		if(indexDesc.hasIndex()){
			...
			for(IndexSpecification indexSpec:indexDesc.getIndexSpecifications()){
				...
				if(indexSpec.getIndexType()==IndexType.CCINDEX){
					admin.deleteColumn(Bytes.toString(indexSpec.getIndexTableName()), 
